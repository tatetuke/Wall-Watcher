...
public class MiniGamePaintToolData : MonoBehaviour
{
    [HideInInspector] public MiniGamePaintToolStatus[] Tools = new MiniGamePaintToolStatus[3];
    public Sprite[] ToolImages;
    private void Start()
    {
        for (int i = 0; i < Tools.Length; i++)
        {
            Tools[i] = new MiniGamePaintToolStatus();
        }
        Tool1Init();
        Tool2Init();
        Tool3Init();
    ...
    private void Tool2Init()
    {
        ...
        Tools[1].descripton = "ながくて鋭い角がかっこいい！\n実はウシ科に属する。\nクリックしたマスとその上下左右4マスを裏返す。";
